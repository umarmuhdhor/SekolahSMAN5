...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Galeri - {{ $schoolProfile['school_name'] }}</title>
        ...
        <main>
            <section class="hero">
                <div class="container hero-box">
                    <p class="eyebrow">Dokumentasi Sekolah</p>
                    <h1>Galeri Kegiatan</h1>
                    <p>Kumpulan dokumentasi visual kegiatan sekolah, prestasi siswa, dan momen penting lainnya.</p>
                </div>
            </section>
            ...
            <section class="content">
                <div class="container">
                    @if ($galleries->isEmpty())
                        <p class="empty-state">Belum ada album galeri yang dipublikasikan.</p>
                    @else
                        <div class="gallery-grid">
                            @foreach ($galleries as $gallery)
                                <article class="gallery-card">
                                    <a href="{{ route('public.galleries.show', ['slug' => $gallery['slug']]) }}">
                                        <img
                                            class="gallery-cover"
                                    ...
                                    <div class="gallery-content">
                                        <p class="gallery-meta">
                                            <span>{{ optional($gallery['published_at'])->translatedFormat('d M Y') ?? '-' }}</span>
                                            <span>•</span>
                                            <span>{{ $gallery['author_name'] }}</span>
                                            <span>•</span>
                                            <span>{{ $gallery['items_count'] }} item</span>
                                        </p>
                                        <h2>
                                            <a href="{{ route('public.galleries.show', ['slug' => $gallery['slug']]) }}">
                                                {{ $gallery['title'] }}
                                        ...
                                        <p class="gallery-summary">
                                            {{ \Illuminate\Support\Str::limit(trim(strip_tags($gallery['description'] ?: 'Album dokumentasi kegiatan sekolah.')), 180) }}
                                        </p>
                                        <a class="gallery-link" href="{{ route('public.galleries.show', ['slug' => $gallery['slug']]) }}">
                                            Lihat Album →
                                        </a>
                                    </div>
                                </article>
                        ...
                        @if ($galleries->hasPages())
                            <div class="pagination">
                                <p class="pagination-summary">
                                    Menampilkan {{ $galleries->firstItem() }}-{{ $galleries->lastItem() }} dari {{ $galleries->total() }} album.
                                </p>
                                <nav class="pagination-links" aria-label="Pagination Galeri">
                                    @if ($galleries->onFirstPage())
                                        <span>«</span>
                                    @else
                                        <a href="{{ $galleries->previousPageUrl() }}" rel="prev">«</a>
                                    @endif
                                    ...
                                    @if ($galleries->hasMorePages())
                                        <a href="{{ $galleries->nextPageUrl() }}" rel="next">»</a>
                                    @else
                                        <span>»</span>
                                    @endif
                                </nav>
