...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{ $announcement->title }} - Pengumuman - {{ $schoolProfile['school_name'] }}</title>
        ...
        <main>
            <section class="hero">
                <div class="container hero-box">
                    <p class="breadcrumb">
                        <a href="{{ route('public.home') }}">Beranda</a> / <a href="{{ route('public.announcements.index') }}">Pengumuman</a> / {{ $announcement->slug }}
                    </p>
                    <h1>{{ $announcement->title }}</h1>
                    <p class="meta">
                        <span>{{ optional($announcement->published_at)->translatedFormat('d M Y H:i') ?? '-' }}</span>
                        <span>•</span>
                ...
                <div class="container">
                    <article class="article">
                        @if (! empty($announcement->excerpt))
                            <p class="article-excerpt">{{ $announcement->excerpt }}</p>
                            <hr class="divider">
                        @endif

                        <p class="article-body">{{ trim((string) $announcement->content) }}</p>

                        <div class="actions">
                            <a href="{{ route('public.announcements.index') }}" class="back-link">← Kembali ke daftar pengumuman</a>
                        </div>
                    </article>
                </div>
            </section>
