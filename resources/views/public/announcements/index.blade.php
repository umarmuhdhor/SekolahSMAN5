...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Pengumuman - {{ $schoolProfile['school_name'] }}</title>
        ...
        <main>
            <section class="hero">
                <div class="container hero-box">
                    ...
                    <p>Informasi resmi sekolah terkait agenda, kebijakan, dan pengumuman penting.</p>
                </div>
            </section>
            ...
            <section class="content">
                <div class="container">
                    @if ($announcements->isEmpty())
                        <p class="empty-state">Belum ada pengumuman yang sedang tayang.</p>
                    @else
                        <div class="news-grid">
                            @foreach ($announcements as $announcement)
                                <article class="news-card">
                                    <p class="news-meta">
                                        <span>{{ optional($announcement->published_at)->translatedFormat('d M Y') ?? '-' }}</span>
                                        <span>•</span>
                                        <span>{{ $announcement->author?->name ?? 'Tim Sekolah' }}</span>
                                    </p>
                                    <h2>
                                        <a href="{{ route('public.announcements.show', ['slug' => $announcement->slug]) }}">
                                            {{ $announcement->title }}
                                        </a>
                                    </h2>
                                    <p class="news-summary">
                                        {{ \Illuminate\Support\Str::limit(trim(strip_tags($announcement->excerpt ?: $announcement->content)), 180) }}
                                    </p>
                                    <a class="news-link" href="{{ route('public.announcements.show', ['slug' => $announcement->slug]) }}">
                                        Baca Detail →
                                    </a>
                                </article>
                            @endforeach
                        ...
                        @if ($announcements->hasPages())
                            <div class="pagination">
                                ...
                                <nav class="pagination-links" aria-label="Pagination Pengumuman">
                                    @if ($announcements->onFirstPage())
                                        <span>«</span>
                                    @else
                                        <a href="{{ $announcements->previousPageUrl() }}" rel="prev">«</a>
                                    @endif
                                    ...
                                    @if ($announcements->hasMorePages())
                                        <a href="{{ $announcements->nextPageUrl() }}" rel="next">»</a>
                                    @else
                                        <span>»</span>
                                    @endif
                                </nav>
